	...
	if(isset($_FILES['hbo']) && !empty($_FILES['hbo'])) {
		...
		if(strtolower($pathinfo['extension']) == "xls") {
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				$durArr1 = explode(":", $slotDuration); //print_r($durArr);
				
				$dateTime1 = mktime($durArr1[0], $durArr1[1], 0, $dateArr[1], $dateArr[0], $dateArr[2]);
				
				$formattedDate1 = date("Y-m-d H:i:s", $dateTime1);        
				
				$seconds = strtotime($formattedDate1) - strtotime($formattedDate);
				
				//$seconds = (($durArr[0] * 60) + $durArr[1]) * 60;
				
				
				$slotName = 	isset($data->sheets[0]['cells'][$i][5]) ? stripslashes($data->sheets[0]['cells'][$i][5]) : "&nbsp;";
				$starring =     isset($data->sheets[0]['cells'][$i][13]) ? stripslashes($data->sheets[0]['cells'][$i][13]) : "&nbsp;";
				$directors = 	isset($data->sheets[0]['cells'][$i][14]) ? stripslashes($data->sheets[0]['cells'][$i][14]) : "&nbsp;";
				$year = 		isset($data->sheets[0]['cells'][$i][12]) ? stripslashes($data->sheets[0]['cells'][$i][12]) : "&nbsp;";
				$genre = 	    isset($data->sheets[0]['cells'][$i][4]) ? stripslashes($data->sheets[0]['cells'][$i][4]) : "&nbsp;";
				
				$duration = $seconds;
				
				$synopsis = 	isset($data->sheets[0]['cells'][$i][11]) ? stripslashes($data->sheets[0]['cells'][$i][11]) : "&nbsp;";
		    ...
		    $searchfolder = array('poster','thumb');
			$convert = array('_','-','');
			foreach($convert as $key=>$val){
			  $searchword[$key] = strtolower(str_replace(" ", $val, $title));
			}
			...
			foreach($searchfolder as $folder){
			 //echo $folder;
				foreach($filesArr as $k=>$v) {
					  ...
					  if(preg_match("/\b$folder\b/i", $v)) {
						if(strpos($v, $searchword[0]) || strpos($v, $searchword[1]) || strpos($v, $searchword[2])){
						  $matchesfiles[$folder] = $v;
						}	
				...
				//echo $matches['thumb'];
				$thumbs = explode('hbosouthasia.com/', $matchesfiles['thumb']);
				$posters = explode('hbosouthasia.com/', $matchesfiles['poster']);
                 //echo $posters[1];
				 //exit;
				$table .= '<td>'.$movieID.'</td>
							<td>'.$seriesID.'</td>
